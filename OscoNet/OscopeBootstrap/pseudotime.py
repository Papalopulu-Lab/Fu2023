...
def _get_peak_time_notrue(data, pt, baseCycleTime=None):
    d = data
    if baseCycleTime is None:
        baseCycle = d
    else:
...
def plot_latent_space_notrue(x_latent):
    fig, ax = plt.subplots(1, 1, figsize=(5, 5))
    ax.scatter(x_latent[:, 0], x_latent[:, 1], s=35)
    r = _get_circle(x_latent)
    circle1 = plt.Circle(r['center'], r['radius'], color='r', alpha=0.1)
    ax.add_artist(circle1)
    return fig, ax
...
def plot_gene_fits_notrue(geneplot, data, commData, pt):
    # Plot gene fits
    if len(geneplot) < 5:
        f, ax = plt.subplots(int(len(geneplot)), 1, figsize=(7, 7), sharey=False, sharex=False)
    else:
    ...
    iterC = 0
    for g in geneplot:
            _plotgene_notrue(data, ax[iterC], pt)
            ax[iterC].set_title(g)
            # add cubic interpolator
            d = data.loc[g]
            idx = np.argsort(pt)
...
def _plotgene_notrue(data, ax, pt, baseCycleTime=None, fFocus=False, f=None):
    d = data
    peakTime = _get_peak_time_notrue(d, pt, baseCycleTime)
    if baseCycleTime is not None:
        ax.axvspan(baseCycleTime[0], baseCycleTime[1], color='blue', alpha=0.5)
        if fFocus:
            ax.set_xlim(baseCycleTime)
    h = ax.scatter(pt, d, s=35)  # plot gene expression
    ax.axvline(peakTime, lw=5, color='green')
    if f is not None:
        f.colorbar(h, ticks=np.arange(15))
    return peakTime
